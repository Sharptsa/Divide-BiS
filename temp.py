...
# Title and text box
wowhead_js_script = '<script>const whTooltips = {colorLinks: true, iconizeLinks: true, renameLinks: true};</script><script src="https://wow.zamimg.com/js/tooltips.js"></script>'
st.set_page_config(layout="wide")
# html(wowhead_js_script)
col = st.columns([0.17, 0.66, 0.17])[1]
col.title('Divide BiS')
col.markdown(wowhead_js_script, unsafe_allow_html=True)
...
def display_df(mask, how='standard', min_glow=252):
    ...
    def item_name_icon_hyperlink_fnc(row):
        ...
        return f'<a href="#" data-wowhead="item={row.item_id}"><img src="{row.icon}" width="22"> {row.item_name}</a>'
