...
def get_items_from_eightyupgrades(url):
    ...
    slots = slots[mask]
    ids = np.array([int(re.findall('item=([0-9]*)', e)[0])
                   for e in elems])[mask]
    names = np.array([re.findall('>(.*?)<', e)[0]
                     for e in elems], dtype='<U50')[mask]

    def convert_tier_to_token(item_slot, item_name):
        assert item_slot in [
            1, 3, 5, 10, 7] and 'Conqueror' in item_name or 'Triumph' in item_name
...
def insert_player_priorities(player):
    ...
    df_priorities = pd.concat(
        [df_priorities, player_bis]).reset_index(drop=True)
    df_res, _, _ = optimize_prios(df_priorities.iloc[:, :-2], resim=False)
    received_mask = (df_priorities.player != player) & (
        df_priorities.received.notna())
    df_res.loc[received_mask,
               'received'] = df_priorities.loc[received_mask, 'received']
