...
def parse_bis_list(locale='en'):
    ...
    bis_path = os.path.join(os.path.dirname(__file__),
                            "../data/players_priorities.xlsx")
    ...
    df_priorities['TOC'] = df_priorities.boss.apply(lambda x: any([val in x for val in
                                                                   ['Beasts', 'Jaraxxus', 'Champions',
                                                                    'Twin', 'Anub', 'Chest', 'Jumelle', 'Bêtes', 'Coffre']]))
    ...
    df_priorities.loc[~df_priorities.lootable & ~
                      df_priorities.legendary, 'received'] = -1.
    df_priorities = df_priorities.sort_values(
        ['TOC', 'boss', 'raid_size', 'hm', 'item_name', 'ilvl', 'player', 'received'],
        ascending=[False, True, False, False, True, False, True, False]
